...
void *cn_thread(void* param)
{
	...
	struct tm tm_msg;
	struct timeval tv_now, tv_msg;
	char *last_read;
	long int pkt_delays[10000], ids_received[10000], diff;
	int milis;
	// Log the Id of the received messages until we press SIGINT
	while (!terminate)
	{
		...
		{	
			// Check if it is our packet sent by the user terminal
			n_bytes = receive_udp(sk, buff, IP_MAXPACKET);
			if (n_bytes  > 0)
			{
				gettimeofday(&tv_now, NULL);
				last_read = strptime(buff, "%Y-%m-%d %H:%M:%S", &tm_msg);
				tv_msg.tv_sec = mktime(&tm_msg);
				sscanf(last_read, "%03d%ld", &milis, &ids_received[packet_counter]);
				diff = (tv_now.tv_sec - tv_msg.tv_sec) * 1000 + (tv_now.tv_usec / 1000 - milis);
				pkt_delays[packet_counter] = diff;
				total_bytes += n_bytes;
				packet_counter++;				
			}
	...
	long int sum = 0;
	for(uint32_t i = 0; i<packet_counter; i++)
	{
		if(i == 0){
			printf("IDs received: %ld", ids_received[i]);
		}else if(ids_received[i]-1 == ids_received[i-1]){
			printf(".");
		}else{
			printf("%ld, %ld", ids_received[i-1], ids_received[i]);
		}
		sum += pkt_delays[i];
	}
	printf("\n\nWe received a total of %ld packets and %ld bytes.\n", packet_counter, total_bytes);
	// printf("The average delay was %f ms\n", (float)sum/(float)(packet_counter*1000));
	return(NULL);
}
